...
// Priority: xtrabackupPath (from flag or config) > XTRABACKUP_PATH env var > PATH lookup
...
func ResolveXtrabackupPath(xtrabackupPath string, requireXbstream bool) (string, string, error) {
	var basePath string

	// Priority 1: Command-line flag or config file
	if xtrabackupPath != "" {
		basePath = xtrabackupPath
	} else {
	...
	var resolvedXtrabackupPath, xbstreamPath string
	var xbstreamDir string

	if info.IsDir() {
		// Directory: append /xtrabackup and /xbstream
		resolvedXtrabackupPath = filepath.Join(basePath, "xtrabackup")
		...
		resolvedXtrabackupPath = basePath
	...
	if err := checkBinaryExecutable(resolvedXtrabackupPath, "xtrabackup"); err != nil {
		return "", "", err
	}
	...
	return resolvedXtrabackupPath, xbstreamPath, nil
}
