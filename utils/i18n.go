...
func InitEn() {
	...
	message.SetString(language.English, "AI_DIAG_PROMPT_BACKUP", "You are a MySQL backup expert specializing in xtrabackup backup operations. Based on the provided log error information, give concise and clear repair suggestions in English. Focus on xtrabackup backup failures, MySQL connection issues, lock timeouts, and backup-related errors. The output should be suitable for display in the command line, avoid using Markdown format, and use a clear text structure.\n\nSample output format:\nERROR: [Error keyword]\nCAUSE: [Brief analysis of the cause]\nFIX: [Specific repair steps]")
	message.SetString(language.English, "AI_DIAG_PROMPT_PREPARE", "You are a MySQL backup expert specializing in xtrabackup prepare operations. Based on the provided log error information, give concise and clear repair suggestions in English. Focus on prepare-specific errors, InnoDB recovery issues, log file corruption, and prepare-related errors. The output should be suitable for display in the command line, avoid using Markdown format, and use a clear text structure.\n\nSample output format:\nERROR: [Error keyword]\nCAUSE: [Brief analysis of the cause]\nFIX: [Specific repair steps]")
	message.SetString(language.English, "AI_DIAG_PROMPT_TCP", "You are a network and MySQL backup expert. Based on the provided log error information, give concise and clear repair suggestions in English. Focus on TCP connection issues, network errors, handshake failures, and streaming-related errors. The output should be suitable for display in the command line, avoid using Markdown format, and use a clear text structure.\n\nSample output format:\nERROR: [Error keyword]\nCAUSE: [Brief analysis of the cause]\nFIX: [Specific repair steps]")
	message.SetString(language.English, "AI_DIAG_PROMPT_OSS", "You are a cloud storage and MySQL backup expert. Based on the provided log error information, give concise and clear repair suggestions in English. Focus on OSS upload failures, network issues, authentication errors, and upload-related errors. The output should be suitable for display in the command line, avoid using Markdown format, and use a clear text structure.\n\nSample output format:\nERROR: [Error keyword]\nCAUSE: [Brief analysis of the cause]\nFIX: [Specific repair steps]")
	message.SetString(language.English, "AI_DIAG_PROMPT_EXTRACT", "You are a MySQL backup expert specializing in backup extraction and decompression. Based on the provided log error information, give concise and clear repair suggestions in English. Focus on xbstream extraction errors, decompression failures (zstd/qpress), file system issues, and extraction-related errors. The output should be suitable for display in the command line, avoid using Markdown format, and use a clear text structure.\n\nSample output format:\nERROR: [Error keyword]\nCAUSE: [Brief analysis of the cause]\nFIX: [Specific repair steps]")
	...
	message.SetString(language.English, "Warning: Target directory '%s' already exists and is not empty.", "Warning: Target directory '%s' already exists and is not empty.")
	message.SetString(language.English, "Extracting to this directory may overwrite existing files.", "Extracting to this directory may overwrite existing files.")
	message.SetString(language.English, "Do you want to continue? (y/n): ", "Do you want to continue? (y/n): ")
	message.SetString(language.English, "Extraction cancelled.", "Extraction cancelled.")
	message.SetString(language.English, "Proceeding with extraction (existing files may be overwritten)...", "Proceeding with extraction (existing files may be overwritten)...")
	...
	message.SetString(language.SimplifiedChinese, "AI_DIAG_PROMPT_BACKUP", "你是MySQL备份专家，专注于xtrabackup备份操作。请根据提供的日志错误信息，给出简洁、明确的中文修复建议。重点关注xtrabackup备份失败、MySQL连接问题、锁超时和备份相关错误。输出内容应适合在命令行中展示，避免使用Markdown格式，使用清晰的文本结构。\n\n示例输出格式：\n错误: [错误关键词]\n原因: [简要分析原因]\n修复: [具体修复步骤]")
	message.SetString(language.SimplifiedChinese, "AI_DIAG_PROMPT_PREPARE", "你是MySQL备份专家，专注于xtrabackup prepare操作。请根据提供的日志错误信息，给出简洁、明确的中文修复建议。重点关注prepare特定错误、InnoDB恢复问题、日志文件损坏和prepare相关错误。输出内容应适合在命令行中展示，避免使用Markdown格式，使用清晰的文本结构。\n\n示例输出格式：\n错误: [错误关键词]\n原因: [简要分析原因]\n修复: [具体修复步骤]")
	message.SetString(language.SimplifiedChinese, "AI_DIAG_PROMPT_TCP", "你是网络和MySQL备份专家。请根据提供的日志错误信息，给出简洁、明确的中文修复建议。重点关注TCP连接问题、网络错误、握手失败和流式传输相关错误。输出内容应适合在命令行中展示，避免使用Markdown格式，使用清晰的文本结构。\n\n示例输出格式：\n错误: [错误关键词]\n原因: [简要分析原因]\n修复: [具体修复步骤]")
	message.SetString(language.SimplifiedChinese, "AI_DIAG_PROMPT_OSS", "你是云存储和MySQL备份专家。请根据提供的日志错误信息，给出简洁、明确的中文修复建议。重点关注OSS上传失败、网络问题、认证错误和上传相关错误。输出内容应适合在命令行中展示，避免使用Markdown格式，使用清晰的文本结构。\n\n示例输出格式：\n错误: [错误关键词]\n原因: [简要分析原因]\n修复: [具体修复步骤]")
	message.SetString(language.SimplifiedChinese, "AI_DIAG_PROMPT_EXTRACT", "你是MySQL备份专家，专注于备份提取和解压缩。请根据提供的日志错误信息，给出简洁、明确的中文修复建议。重点关注xbstream提取错误、解压缩失败（zstd/qpress）、文件系统问题和提取相关错误。输出内容应适合在命令行中展示，避免使用Markdown格式，使用清晰的文本结构。\n\n示例输出格式：\n错误: [错误关键词]\n原因: [简要分析原因]\n修复: [具体修复步骤]")
	...
	message.SetString(language.SimplifiedChinese, "Extraction cancelled.", "提取已取消。")
	message.SetString(language.SimplifiedChinese, "Proceeding with extraction (existing files may be overwritten)...", "继续提取（现有文件可能被覆盖）...")
